...
- **Desktop Preview Mode** File browser supports `ViewMode.gridPreview` with a resizable right preview pane (video/image via existing viewers, PDF via `pdfx` `PdfView`); not available on mobile and shows a placeholder for network paths.
