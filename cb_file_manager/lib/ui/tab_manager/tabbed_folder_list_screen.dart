...
class _TabbedFolderListScreenState extends State<TabbedFolderListScreen> {
  ...
  Widget _buildBody(BuildContext context, FolderListState state) {
    ...
    if (state.error != null) {
      bool isAdminError =
          state.error!.toLowerCase().contains('access denied') ||
              state.error!.toLowerCase().contains('administrator privileges');
              ...
              onPressed: () {
                ...
                try {
                  ...
                  if (parentPath != _currentPath) {
                    ...
                    // If we're at root level, try to navigate to the drive listing
